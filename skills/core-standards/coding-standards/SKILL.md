...
description: Portable TypeScript engineering standards. Use when writing or reviewing TypeScript, feature slices, packages, domain models, parsers, typed errors, async workflows, tests, exports, Cloudflare runtime code, or Effect responsibilities; use when another skill needs the coding standards.
---
...
- Use this skill as the source of truth for engineering standards and topic routing.
- Use `../code-review/` for review-only analysis of a concrete diff, branch, PR, or working tree.
- Use `../improve-codebase-architecture/` for planning-only architecture scans across a codebase or subsystem.
- Use `../tech-spec/` to turn a chosen design, issue, PRD, or architecture candidate into an implementation-ready typed spec.
- Use `../tdd/` for red-green-refactor implementation loops and test-first behavior slices.
- Use `../build-intent-layer/` when the durable rule belongs in agent instructions rather than product code.
...
- Organize code feature-slice first; layers exist inside slices when they earn their keep.
- Give every data value one clear owner as it moves from persistence through services, protocol boundaries, caches, and UI.
...
- Features expose intentional public contracts; other features do not import internals by convenience.
...
| Feature-slice layout, app/package boundaries, package promotion, public exports, feature dependency direction, schema ownership, protocol subpaths, frontend/backend slice shape | [`FEATURE_SLICE_ARCHITECTURE.md`](FEATURE_SLICE_ARCHITECTURE.md) |
...
| Data ownership, reads vs mutations, DB/service/API/MCP/frontend flow, server state, local state, cache policy, data fetching, invalidation, fallbacks, optimistic updates | [`DATA_FLOW_AND_STATE.md`](DATA_FLOW_AND_STATE.md) |
