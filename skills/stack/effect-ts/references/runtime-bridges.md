...
const transaction = <A, E>(
  ...
  Effect.tryPromise({
    try: () =>
      ...
      if (cause instanceof TransactionFailure) return cause.error
      return new PersistenceError({ operation: "transaction", cause })
    },
  })
```
...
`FiberSet.makeRuntime` is available on current v4 lines but must still be checked against the target pin. Use an explicit `FiberSet` plus `FiberSet.runtime` when the service must inspect, join, clear, or count fibers.
