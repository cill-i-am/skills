...
With current `@effect/vitest`, `it.effect` and `it.live` create and close a Scope for each test. Do not wrap the body in `Effect.scoped` solely to obtain Scope.
...
Use a nested Scope deliberately when the assertion needs to run after that Scope closes. Do not depend on assertions running after the test itself has returned.
